...
function createHomepageGoogleMap(_latitude,_longitude,json){
    $.get(window.public_path+"external/_infobox.js", function() {
        gMap();
    });
    function gMap(){
        ...
        $('.geolocation').on("click", function() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(successFixed);
            } else {
        ...
        function successFixed(position) {
            var locationCenter = new google.maps.LatLng(19.4333,-99.1333);//position.coords.latitude, position.coords.longitude);
            map.setCenter( locationCenter );
            map.setZoom(14);
...
function createHomepageOSM(_latitude,_longitude,json,mapProvider,container){
    ...
    function osmMap(){
        var localNameContainer = 'map';
        if(container){
            localNameContainer = container;
        }
        var map = L.map(localNameContainer, {
        ...
        for (var i = 0; i < json.data.length; i++) {
			...
			var imagepath = window.public_path+'icons/sports/relaxing-sports/weights.png';
				
            //if( json.data[i].category ) var icon = '<img src="' + imagepath +  '">';
            //else icon = '';
            var icon = '<img src="' + imagepath +  '">';

            if( json.data[i].color ) var color = json.data[i].color;
            else color = '';
            //var d = new Date();
            //var idDiv = "marker_"+i+'_'+addZero(d.getHours())+'_'+addZero(d.getMinutes());
            var markerContent =
                '<div class="map-marker ' + color + ' idContainer" data-id="'+json.data[i].id +'">' +
                    '<div class="icon">' +
                    icon +
                    '</div>' +
                '</div>';
            ...
            var _icon = L.divIcon({
                ...
                popupAnchor:  [130, -28],
                className: '',
                idfac : json.data[i].id
            });
			var distanceinKM = 20;
			if ( distanceinKM >= getDistanceFromLatLonInKm(_latitude, _longitude, json.data[i].lat, json.data[i].lon) )
			{
				var title = json.data[i].title;
				var marker = L.marker(new L.LatLng( json.data[i].lat, json.data[i].lon ), {
					title: title,
					icon: _icon
				});
                ...
                //infoboxContent.setAttribute("id", idDiv);
                //infoboxContent.className = infoboxContent.className + " otherclass";
                //marker._icon.className += idDiv;
				marker.bindPopup(
				 	drawInfobox(category, infoboxContent, json, i,window.profile_path,marker)
				);
				...
				marker.on('popupopen', function () {
                    ...
                    var id = this._icon.getElementsByClassName('idContainer')[0].getAttribute('data-id');
                    var sender = this._popup;
                    this.bindPopup(
                        drawInfoboxajax(id,window.profile_path,sender)
                    );
				});
				marker.on('popupclose', function () {
                    // var temporalClass = this._icon.className;
                    // var pieces = temporalClass.split(' ');
                    // var classTarget = "";
                    // for(i=0;i<pieces.length;i++){
                    //     var postition = pieces[i].indexOf('marker_');
                    //     if(position >= 0){
                    //         classTarget = pieces[i];
                    //     }
                    // }
        ...
        $('.results .item').hover(
            function(){
                loadedMarkers[ $(this).attr('id') - 1 ].options.icon.options.className = 'leaflet-marker-icon leaflet-zoom-animated leaflet-clickable marker-loaded marker-active';
            },
            function() {
                loadedMarkers[ $(this).attr('id') - 1 ].options.icon.options.className = 'leaflet-marker-icon leaflet-zoom-animated leaflet-clickable marker-loaded';
            }
        );
...
function pushItemsToArray(json, a, category, visibleItemsArray){
    var itemPrice;
    var imageLocal = '';
    if(json.data[a].picture){
        imageLocal = '<img src="' + json.data[a].picture + '" alt="">';
    }else{
        imageLocal = '<img src="'+window.public_path+'/img/items/real-estate/12.jpg" alt="" class="mCS_img_loaded">';
    }
    ...
    visibleItemsArray.push(
        '<li>' +
            '<div class="item facility" id="' + json.data[a].id + '">' +
                '<a href="#" class="image go_profile">' +
                    '<div class="inner">' +
                ...
                '</a>' +
                '<div class="wrapper">' +
                    '<a href="#" id="' + json.data[a].id + '" class="go_profile"><h3>' + json.data[a].title + '</h3></a>' +
                    ...
                    '<div class="compare-wrapper">' + 
                        '<a href="javascript:void();" class="compare-link-'+ json.data[a].id + '" data-id="' + json.data[a].id + '" data-name="' + json.data[a].title + '">Compare</a>' + 
                    '</div>' +  
                '</div>' +
            '</div>' +
        '</li>'
...
// Haversine formula adapted from http://stackoverflow.com/questions/27928/how-do-i-calculate-distance-between-two-latitude-longitude-points
// to calculate the distance between two latitude-longitude-points in kilometres.
