...
function drawOwlCarousel(_rtl){
    $.getScript( window.public_path+"js/owl.carousel.min.js", function( data, textStatus, jqxhr ) {
...
function quickView(id){
    $.ajax({
        ...
        success: function (data) {
            if (id === 999) {
                var wrapper = $(data).find('.modal-wrapper');
                if(wrapper){
                    $(wrapper).html('<div style="background:#fff;max-height:150px;"><h3>You only can compare up to three gyms</h3></div>');
